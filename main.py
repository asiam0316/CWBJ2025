...
st.sidebar.header("あなたについて教えてください")
...
sidebar_city = st.sidebar.text_input("農作業する市区町村を入力してください（例：神戸市）", "兵庫県丹波市")
...
if submitted:
    if not details.strip():
        ...
        try:
            # --- 天気を取得 ---
            weather = get_weather(sidebar_city, os.getenv("OPENWEATHERMAP_API_KEY"))
            st.info(f"""
            天気：{weather['condition']}
            気温：{weather['temperature']}°C
            湿度：{weather['humidity']}%
            風速：{weather['wind_speed']} m/s
            降水確率：{weather['rain_probability']} mm
            """)            
...
# 出力形式
# ・{sidebar_city}の{today.strftime('%Y年%m月%d日')}の天気、湿度、風速、降水確率を検索して表示
・年齢相応の注意点をわかりやすく具体的に
・3～5個程度のリスクと注意点を箇条書き
・リスクと注意点は、作業内容に関連するものを選択してください。
・リスクを記載した後で改行して注意点を記載してください。
・注意点は、「〇〇に注意」「△△を確認する」などの形式で回答してください。
・農林水産省、農機具メーカー、農協などの危険予知や農作業安全資料を参考にしてください。
"""
